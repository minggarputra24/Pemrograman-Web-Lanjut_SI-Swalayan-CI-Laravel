...
public function getPenjualan($kdpenjualan = null)
    {
        if ($kdpenjualan === null) {
            return $this->db->get('penjualan')->result_array();
        } else {
            return $this->db->get_where('penjualan', ['kdpenjualan' => $kdpenjualan])->result_array();
        }
    }

    public function createPenjualan($data)
    {
        // $this->db->create('totalHarga', ['totalHarga' => $total]);
        $this->db->insert('penjualan', $data);
        return $this->db->affected_rows();
    }

    public function updatePenjualan($data, $kdpenjualan)
    {
        $this->db->update('penjualan', $data, ['kdpenjualan' => $kdpenjualan]);
        return $this->db->affected_rows();
    }

    public function deletePenjualan($kdpenjualan)
    {
        $this->db->delete('penjualan', ['kdpenjualan' => $kdpenjualan]);
        return $this->db->affected_rows();
    }
